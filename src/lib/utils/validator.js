...
class Validator {
  ...
  constructor(options = {}) {
    // logger を必須にする
    if (!options.logger) {
      throw new Error('Logger instance is required in Validator options.');
    }
  ...
  validateTaskInput(taskData) {
    ...
    if (taskData.priority !== undefined) {
      if (
        !Number.isInteger(taskData.priority) || // 整数かどうかをチェック
        taskData.priority < 1 ||
        taskData.priority > 5
      ) {
    ...
    if (taskData.estimated_hours !== undefined) {
      if (
        typeof taskData.estimated_hours !== 'number' ||
    ...
    if (taskData.progress_percentage !== undefined) {
      if (
        typeof taskData.progress_percentage !== 'number' ||
    ...
    if (taskData.dependencies) {
      if (!Array.isArray(taskData.dependencies)) {
        ...
        for (const dependency of taskData.dependencies) {
          ...
          if (
            dependency.type &&
            !['strong', 'weak'].includes(dependency.type)
          ) {
            errors.push(
  ...
  validateFeedbackInput(feedbackData) {
    ...
    if (
      loop.feedback_status &&
      !['open', 'in_progress', 'resolved', 'wontfix'].includes(
        loop.feedback_status
      )
    ) {
      errors.push(
