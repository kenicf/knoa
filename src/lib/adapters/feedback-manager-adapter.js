...
class FeedbackManagerAdapter extends BaseAdapter {
  ...
  validateFeedback(feedback, context = null) {
    try {
      ...
      return this._handleError(error, 'validateFeedback', context, {
        feedback,
      });
    }
  }
  ...
  async getPendingFeedback(context = null) {
    try {
      // コンテキストがない場合は作成
      const operationContext =
        context || this._createContext('getPendingFeedback');
      ...
      if (feedback) {
        this._emitEvent(
          'feedback',
          'pending_feedback_retrieved',
          {
            feedbackCount: feedback.feedback_loop
              ? feedback.feedback_loop.feedback_items.length
              : 0,
  ...
  async getFeedbackByTaskId(taskId, context = null) {
    try {
      // コンテキストがない場合は作成
      const operationContext =
        context || this._createContext('getFeedbackByTaskId', { taskId });
      ...
      return this._handleError(error, 'getFeedbackByTaskId', context, {
        taskId,
      });
    }
  }
  ...
  async createNewFeedback(taskId, attempt, context = null) {
    try {
      ...
      const operationContext =
        context ||
        this._createContext('createNewFeedback', { taskId, attempt });
      ...
      const feedback = await this.manager.createNewFeedback(
        taskId,
        attempt || 1
      );
      ...
      return this._handleError(error, 'createNewFeedback', context, {
        taskId,
        attempt,
      });
    }
  ...
  async collectTestResults(taskId, testCommand, testTypes, context = null) {
    try {
      ...
      const operationContext =
        context ||
        this._createContext('collectTestResults', {
          taskId,
          testCommand,
          testTypes,
        });
      ...
      const results = await this.manager.collectTestResults(
        taskId,
        testCommand,
        testTypes || []
      );
      ...
      return this._handleError(error, 'collectTestResults', context, {
        taskId,
        testCommand,
        testTypes,
      });
  ...
  async prioritizeFeedback(feedback, context = null) {
    try {
      ...
      const operationContext =
        context ||
        this._createContext('prioritizeFeedback', {
          feedbackId: feedback.id,
      ...
      const prioritizedFeedback =
        await this.manager.prioritizeFeedback(feedback);
      ...
      return this._handleError(error, 'prioritizeFeedback', context, {
        feedback,
      });
    }
  }
  ...
  async updateFeedbackStatus(feedback, newStatus, context = null) {
    try {
      ...
      const operationContext =
        context ||
        this._createContext('updateFeedbackStatus', {
          feedbackId: feedback.id,
          taskId: feedback.task_id,
          newStatus,
        });
      ...
      const updatedFeedback = await this.manager.updateFeedbackStatus(
        feedback,
        newStatus
      );
      ...
      return this._handleError(error, 'updateFeedbackStatus', context, {
        feedback,
        newStatus,
      });
    }
  ...
  async integrateFeedbackWithSession(feedbackId, sessionId, context = null) {
    try {
      ...
      const operationContext =
        context ||
        this._createContext('integrateFeedbackWithSession', {
          feedbackId,
          sessionId,
        });

      this._validateParams({ feedbackId, sessionId }, [
        'feedbackId',
        'sessionId',
      ]);

      const result = await this.manager.integrateFeedbackWithSession(
        feedbackId,
        sessionId
      );
      ...
      return this._handleError(error, 'integrateFeedbackWithSession', context, {
        feedbackId,
        sessionId,
      });
    }
  ...
  async integrateFeedbackWithTask(feedbackId, taskId, context = null) {
    try {
      ...
      const operationContext =
        context ||
        this._createContext('integrateFeedbackWithTask', {
          feedbackId,
          taskId,
        });
      ...
      const result = await this.manager.integrateFeedbackWithTask(
        feedbackId,
        taskId
      );
      ...
      return this._handleError(error, 'integrateFeedbackWithTask', context, {
        feedbackId,
        taskId,
      });
    }
  ...
  async generateFeedbackMarkdown(taskId, context = null) {
    try {
      // コンテキストがない場合は作成
      const operationContext =
        context || this._createContext('generateFeedbackMarkdown', { taskId });
      ...
      return this._handleError(error, 'generateFeedbackMarkdown', context, {
        taskId,
      });
    }
  }
