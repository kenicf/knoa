  ...
  describe('イベント連携', () => {
    ...
    beforeEach(() => {
      ...
      emitter.on('task:created', (data) => {
        mockLogger.info(
          `[EVENT] タスク作成イベント受信: ${data.id} - ${data.title}`
        );
    ...
    test('イベント連携が正しく動作する', () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        expect.stringContaining('[EVENT] タスク作成イベント受信')
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        expect.stringContaining('[EVENT] タスク更新イベント受信')
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        expect.stringContaining('[EVENT] タスクステータス変更イベント受信')
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        expect.stringContaining('[NOTIFICATION] notification:created')
      );
