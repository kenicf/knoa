...
describe('アダプター統合テスト', () => {
  ...
  test('イベントリスナーを使用したコンポーネント間の連携', async () => {
    ...
    // イベントの数を確認
    const taskCreatedEvents = emittedEvents.filter(
      (e) => e.name === 'task:task_created'
    );
    const sessionCreatedEvents = emittedEvents.filter(
      (e) => e.name === 'session:session_created'
    );
    const taskAddedEvents = emittedEvents.filter(
      (e) => e.name === 'session:task_added'
    );
    const feedbackCreatedEvents = emittedEvents.filter(
  ...
  test('エラー発生時のイベント連鎖の中断', async () => {
    ...
    eventEmitter.on('app:error', (errorData) => {
      ...
      mockLogger.error(
        `[ERROR] ${errorData.component}.${errorData.operation}: ${errorData.message}`,
        errorData
      );
    });
