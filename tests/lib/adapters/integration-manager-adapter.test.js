...
describe('IntegrationManagerAdapter', () => {
  ...
  beforeEach(() => {
    ...
    mockIntegrationManager = {
      ...
      resolveFeedback: jest
        .fn()
        .mockImplementation((feedbackId, resolution) => ({
          id: feedbackId,
          resolved: true,
          resolution,
          timestamp: new Date().toISOString(),
  ...
  describe('initializeWorkflow', () => {
    test('ワークフローを初期化し、イベントを発行する', async () => {
      ...
      expect(mockIntegrationManager.initializeWorkflow).toHaveBeenCalledWith(
        projectData
      );
      ...
      // イベント発行のテスト
      const workflowInitializedEvent = emittedEvents.find(
        (e) => e.name === 'integration:workflow_initialized'
      );
  ...
  describe('startSession', () => {
    test('セッションを開始し、イベントを発行する', async () => {
      ...
      // イベント発行のテスト
      const sessionStartedEvent = emittedEvents.find(
        (e) => e.name === 'integration:session_started'
      );
  ...
  describe('endSession', () => {
    test('セッションを終了し、イベントを発行する', async () => {
      ...
      // イベント発行のテスト
      const sessionEndedEvent = emittedEvents.find(
        (e) => e.name === 'integration:session_ended'
      );
  ...
  describe('createTask', () => {
    test('タスクを作成し、イベントを発行する', async () => {
      ...
      const taskCreatedEvent = emittedEvents.find(
        (e) => e.name === 'integration:task_created'
      );
  ...
  describe('updateTaskStatus', () => {
    test('タスク状態を更新し、イベントを発行する', async () => {
      ...
      expect(mockIntegrationManager.updateTaskStatus).toHaveBeenCalledWith(
        taskId,
        status
      );
      expect(result).toMatchObject({
      ...
      // イベント発行のテスト
      const taskStatusUpdatedEvent = emittedEvents.find(
        (e) => e.name === 'integration:task_status_updated'
      );
  ...
  describe('collectFeedback', () => {
    test('フィードバックを収集し、イベントを発行する', async () => {
      ...
      expect(mockIntegrationManager.collectFeedback).toHaveBeenCalledWith(
        taskId,
        feedbackData
      );
      expect(result).toMatchObject({
      ...
      // イベント発行のテスト
      const feedbackCollectedEvent = emittedEvents.find(
        (e) => e.name === 'integration:feedback_collected'
      );
    ...
    test('エラー時に適切に処理する', async () => {
      ...
      const result = await adapter.collectFeedback('T001', {
        content: 'テストフィードバック',
      });
  ...
  describe('resolveFeedback', () => {
    test('フィードバックを解決し、イベントを発行する', async () => {
      ...
      expect(mockIntegrationManager.resolveFeedback).toHaveBeenCalledWith(
        feedbackId,
        resolution
      );
      expect(result).toMatchObject({
      ...
      // イベント発行のテスト
      const feedbackResolvedEvent = emittedEvents.find(
        (e) => e.name === 'integration:feedback_resolved'
      );
  ...
  describe('syncComponents', () => {
    test('コンポーネント間の同期を実行し、イベントを発行する', async () => {
      ...
      // イベント発行のテスト
      const componentsSyncedEvent = emittedEvents.find(
        (e) => e.name === 'integration:components_synced'
      );
      expect(componentsSyncedEvent).toBeDefined();
      expect(componentsSyncedEvent.data.components).toEqual([
        'session',
        'task',
        'feedback',
      ]);
  ...
  describe('generateReport', () => {
    test('レポートを生成し、イベントを発行する', async () => {
      ...
      expect(mockIntegrationManager.generateReport).toHaveBeenCalledWith(
        options
      );
      expect(result).toMatchObject({
        reportId: 'R001',
      ...
      // イベント発行のテスト
      const reportGeneratedEvent = emittedEvents.find(
        (e) => e.name === 'integration:report_generated'
      );
  ...
  describe('startPeriodicSync', () => {
    test('定期同期を開始し、イベントを発行する', async () => {
      ...
      expect(mockIntegrationManager.startPeriodicSync).toHaveBeenCalledWith(
        interval
      );
      expect(result).toMatchObject({
        started: true,
      ...
      // イベント発行のテスト
      const periodicSyncStartedEvent = emittedEvents.find(
        (e) => e.name === 'integration:periodic_sync_started'
      );
  ...
  describe('stopPeriodicSync', () => {
    test('定期同期を停止し、イベントを発行する', async () => {
      ...
      // イベント発行のテスト
      const periodicSyncStoppedEvent = emittedEvents.find(
        (e) => e.name === 'integration:periodic_sync_stopped'
      );
