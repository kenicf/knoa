...
describe('Repository', () => {
  ...
  describe('constructor', () => {
    test('should throw error if logger is not provided', () => {
      // logger が必須になったため、logger 不足のエラーを期待
      expect(() => new Repository(mockDeps.storageService, 'test')).toThrow(
        'Logger instance is required' // エラーメッセージは実装に合わせる必要あり
      );
    });
    ...
    test('should create repository with default options', () => {
      ...
      expect(
        mockDeps.storageService.ensureDirectoryExists
      ).toHaveBeenCalledWith('ai-context/tests');
      expect(
        mockDeps.storageService.ensureDirectoryExists
      ).toHaveBeenCalledWith('ai-context/tests/test-history');
    });
  ...
  describe('getAll', () => {
    ...
    test('should throw error if readJSON fails', async () => {
      ...
      await expect(repository.getAll()).rejects.toThrow(
        'Failed to get all tests: Read error'
      );
    });
  });
  ...
  describe('create', () => {
    test('should validate entity if validator is provided', async () => {
      ...
      const repoWithValidator = new Repository(
        mockDeps.storageService,
        'test',
        { validator }
      );
    ...
    test('should throw ValidationError if validation fails', async () => {
      ...
      await expect(repoWithValidator.create(mockEntity)).rejects.toThrow(
        ValidationError
      );
      expect(validator.validate).toHaveBeenCalledWith(mockEntity);
    });
  ...
  describe('update', () => {
    test('should throw NotFoundError if entity not found', async () => {
      ...
      await expect(
        repository.update('test2', { name: 'Updated' })
  ...
  describe('createMany', () => {
    ...
    test('should throw error if dataArray is not an array', async () => {
      await expect(repository.createMany('not an array')).rejects.toThrow(
  ...
  describe('updateMany', () => {
    test('should update multiple entities', async () => {
      ...
      expect(repository.update).toHaveBeenCalledWith('test1', {
        name: 'Updated Test 1',
      });
      expect(repository.update).toHaveBeenCalledWith('test2', {
        name: 'Updated Test 2',
      });
    });

    test('should throw error if updateArray is not an array', async () => {
      await expect(repository.updateMany('not an array')).rejects.toThrow(
        'updateArray must be an array'
      );
    });
  ...
  describe('deleteMany', () => {
    ...
    test('should throw error if ids is not an array', async () => {
      await expect(repository.deleteMany('not an array')).rejects.toThrow(
