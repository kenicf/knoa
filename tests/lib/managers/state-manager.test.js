...
describe('StateManager', () => {
  ...
  describe('状態遷移', () => {
    test('有効な状態遷移が成功すること', () => {
      ...
      expect(
        stateManager.canTransitionTo(stateManager.states.INITIALIZED)
      ).toBe(true);
      expect(stateManager.transitionTo(stateManager.states.INITIALIZED)).toBe(
        true
      );
      expect(stateManager.getCurrentState()).toBe(
      ...
      expect(
        stateManager.canTransitionTo(stateManager.states.SESSION_STARTED)
      ).toBe(true);
      expect(
        stateManager.transitionTo(stateManager.states.SESSION_STARTED)
      ).toBe(true);
      expect(stateManager.getCurrentState()).toBe(
        stateManager.states.SESSION_STARTED
      );
    });
    ...
    test('無効な状態遷移が失敗すること', () => {
      ...
      expect(
        stateManager.canTransitionTo(stateManager.states.TASK_IN_PROGRESS)
      ).toBe(false);
      expect(() => {
  ...
  describe('前の状態', () => {
    test('前の状態を取得できること', () => {
      stateManager.transitionTo(stateManager.states.INITIALIZED);
      expect(stateManager.getPreviousState()).toBe(
        stateManager.states.UNINITIALIZED
      );
