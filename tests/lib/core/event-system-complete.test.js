...
describe('イベントシステムの完全実装', () => {
  describe('EnhancedEventEmitter', () => {
    ...
    test('標準化されたイベント名を生成できる', () => {
      expect(emitter.createStandardEventName('component', 'action')).toBe(
        'component:action'
      );
      expect(emitter.createStandardEventName('TASK', 'Created')).toBe(
        'task:created'
      );
      expect(
        emitter.createStandardEventName('session-manager', 'start_session')
      ).toBe('sessionmanager:startsession');
    });
    ...
    test('非標準のイベント名を警告する', () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining('非標準のイベント名')
      );
    ...
    test('非同期で標準化されたイベントを発行できる', async () => {
      ...
      await emitter.emitStandardizedAsync('component', 'action', {
        data: 'test',
      });
  ...
  describe('EventCatalog', () => {
    ...
    test('実際のイベントカタログが正しく読み込まれる', () => {
      ...
      expect(
        eventCatalog.getEventDefinition('feedback:collected')
      ).toBeDefined();
    });
  });
  ...
  describe('統合テスト', () => {
    ...
    test('イベント履歴が正しく記録される', () => {
      ...
      const event1Entries = history.filter(
        (entry) => entry.event === 'test:event1'
      );
      ...
      const event2Entries = history.filter(
        (entry) => entry.event === 'test:event2'
      );
