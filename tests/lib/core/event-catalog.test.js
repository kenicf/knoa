...
describe('EventCatalog', () => {
  ...
  describe('実際のイベントカタログ', () => {
    ...
    test('標準イベントが登録されている', () => {
      ...
      expect(
        eventCatalog.getEventDefinition('task:statusChanged')
      ).toBeDefined();
      ...
      expect(
        eventCatalog.getEventDefinition('feedback:resolved')
      ).toBeDefined();
      ...
      // ストレージ関連イベント
      expect(
        eventCatalog.getEventDefinition('storage:fileCreated')
      ).toBeDefined();
      expect(
        eventCatalog.getEventDefinition('storage:fileUpdated')
      ).toBeDefined();

      // システム関連イベント
      expect(
        eventCatalog.getEventDefinition('system:initialized')
      ).toBeDefined();
    ...
    test('イベント定義が必要な情報を含んでいる', () => {
      ...
      const sessionStartedEvent =
        eventCatalog.getEventDefinition('session:started');
