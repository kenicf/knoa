...
describe('session CLI', () => {
  ...
  test('startコマンドでセッションが開始される', async () => {
    ...
    // セッションが開始されることを確認
    expect(
      mocks.adapters.sessionManagerAdapter.createNewSession
    ).toHaveBeenCalled();
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      '新しいセッションを開始します'
    );
  ...
  test('endコマンドでセッションが終了される', async () => {
    ...
    // セッションが終了されることを確認
    expect(
      mocks.adapters.sessionManagerAdapter.endSession
  ...
  test('listコマンドでセッション一覧が表示される', async () => {
    ...
    // セッション一覧が取得されることを確認
    expect(
      mocks.adapters.sessionManagerAdapter.getAllSessions
    ).toHaveBeenCalled();
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      'セッション一覧を表示します'
    );
  ...
  test('infoコマンドでセッション情報が表示される', async () => {
    ...
    // セッション情報が取得されることを確認
    expect(
      mocks.adapters.sessionManagerAdapter.getSession
    ).toHaveBeenCalledWith('S001');
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      'セッション情報を表示します'
    );
  ...
  test('infoコマンドでセッションIDが指定されていない場合はエラーメッセージが表示される', async () => {
    ...
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      '使用方法: node session.js info <session-id>'
    );
  });
  ...
  test('exportコマンドでセッション情報がエクスポートされる', async () => {
    ...
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      'セッション情報を output.json にエクスポートしました'
    );
  });
  ...
  test('importコマンドでセッション情報がインポートされる', async () => {
    ...
    expect(
      mocks.adapters.sessionManagerAdapter.importSession
    ).toHaveBeenCalled();
