...
describe('feedback CLI', () => {
  ...
  test('collectコマンドでフィードバックが収集される', async () => {
    ...
    // フィードバックが収集されることを確認
    expect(
      mocks.adapters.feedbackManagerAdapter.collectTestResults
    ).toHaveBeenCalledWith('T001', 'テスト結果');
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      'フィードバックを収集します'
    );
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
  ...
  test('collectコマンドでタスクIDとテスト結果が指定されていない場合はエラーメッセージが表示される', async () => {
    ...
    // エラーメッセージが表示されることを確認
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      'タスクIDとテスト結果を指定してください'
    );
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      '使用方法: node feedback.js collect <task-id> <test-results>'
    );
  });
  ...
  test('listコマンドでフィードバック一覧が表示される', async () => {
    ...
    expect(
      mocks.adapters.feedbackManagerAdapter.getFeedbackByTaskId
  ...
  test('listコマンドでタスクIDが指定されていない場合はエラーメッセージが表示される', async () => {
    ...
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      '使用方法: node feedback.js list <task-id>'
    );
  });
  ...
  test('updateコマンドでフィードバック状態が更新される', async () => {
    ...
    expect(
      mocks.adapters.feedbackManagerAdapter.updateFeedbackStatus
    ).toHaveBeenCalledWith('F001', 'resolved');
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      'フィードバック状態を更新します'
    );
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
  ...
  test('updateコマンドでフィードバックIDと状態が指定されていない場合はエラーメッセージが表示される', async () => {
    ...
    // エラーメッセージが表示されることを確認
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      'フィードバックIDと状態を指定してください'
    );
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      '使用方法: node feedback.js update <feedback-id> <status>'
    );
  });
  ...
  test('generateコマンドでフィードバックレポートが生成される', async () => {
    ...
    expect(
      mocks.adapters.feedbackManagerAdapter.generateFeedbackMarkdown
    ).toHaveBeenCalledWith('T001');
    ...
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      'フィードバックレポートを report.md に保存しました'
    );
  });
  ...
  test('generateコマンドでタスクIDが指定されていない場合はエラーメッセージが表示される', async () => {
    ...
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      '使用方法: node feedback.js generate <task-id> [output-path]'
    );
  });
  ...
  test('prioritizeコマンドでフィードバックが優先順位付けされる', async () => {
    ...
    expect(
      mocks.adapters.feedbackManagerAdapter.prioritizeFeedback
    ).toHaveBeenCalledWith('T001');
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      'フィードバックを優先順位付けします'
    );
  ...
  test('linkコマンドでフィードバックがGitコミットに関連付けられる', async () => {
    ...
    expect(
      mocks.adapters.feedbackManagerAdapter.linkFeedbackToGitCommit
    ).toHaveBeenCalledWith('F001', 'abc123');
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      'フィードバックをGitコミットに関連付けます'
    );
  });
  ...
  test('linkコマンドでフィードバックIDとコミットハッシュが指定されていない場合はエラーメッセージが表示される', async () => {
    ...
    // エラーメッセージが表示されることを確認
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      'フィードバックIDとコミットハッシュを指定してください'
    );
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      '使用方法: node feedback.js link <feedback-id> <commit-hash>'
    );
  });
  ...
  test('linkSessionコマンドでフィードバックがセッションに関連付けられる', async () => {
    ...
    expect(
      mocks.adapters.feedbackManagerAdapter.linkFeedbackToSession
    ).toHaveBeenCalledWith('F001', 'S001');
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      'フィードバックをセッションに関連付けます'
    );
  });
  ...
  test('integrateTaskコマンドでフィードバックがタスクと統合される', async () => {
    ...
    expect(
      mocks.adapters.feedbackManagerAdapter.integrateFeedbackWithTask
    ).toHaveBeenCalledWith('F001', 'T001');
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      'フィードバックをタスクと統合します'
    );
  });
  ...
  test('integrateSessionコマンドでフィードバックがセッションと統合される', async () => {
    ...
    expect(
      mocks.adapters.feedbackManagerAdapter.integrateFeedbackWithSession
    ).toHaveBeenCalledWith('F001', 'S001');
    expect(consoleCapture.consoleOutput.join('\n')).toContain(
      'フィードバックをセッションと統合します'
    );
